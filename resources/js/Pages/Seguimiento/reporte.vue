        ...
        <template #header>
            ...
            <div id="chartContainerWrapper">
                <div v-for="archivo in archivos" :key="archivo.id" id="chartContainer">
                    <h3 class="texto">{{ archivo.nombre_categoria }} , {{ archivo.total_archivos }}</h3> <!-- Mostrar el nombre del archivo -->
                    <div class="bar revisado" :style="{ height: calcularAltura(archivo.cantidad_archivos_revisados) + 'px' }">
                        <span>{{ archivo.cantidad_archivos_revisados }}</span>
                    </div>
                    <div class="bar sin-revisar" :style="{ height: calcularAltura(archivo.cantidad_archivos_sin_revisar) + 'px' }">
...
.bar {
    width: 140px; /* Ancho de las barras */
    background-color: #6c5ce7; /* Color base de las barras */
    margin-right: 20px; /* Espacio entre barras */
    transition: height 0.5s ease-in-out, width 0.5s ease-in-out; /* Animación de altura y ancho */
    border-radius: 5px;
    overflow: hidden;
    position: relative;
}
...
.bar.revisado {
    background-color: #74b9ff; /* Azul para elementos revisados */
}

.bar.sin-revisar {
    background-color: #ff7675; /* Rojo para elementos sin revisar */
}
</style>
